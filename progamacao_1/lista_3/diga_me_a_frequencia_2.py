while True:
    try:
        texto = input()
        valores_ascii = []

        # armazenando caracteres na lista valores_ascii na
        # forma [[frequencia, valor]]
        # ex.: [[1, 94], [4, 80]]
        for char in texto:
            if len(valores_ascii) != 0:
                valor_existe = False
                index = 0

                for i in range(len(valores_ascii)):
                    if ord(char) == valores_ascii[i][1]:
                        valor_existe = True
                        valores_ascii[i][0] += 1
                        index = i

                if not valor_existe:
                    valores_ascii += [[1, ord(char)]]
                    index = len(valores_ascii)-1
            else:
                valores_ascii += [[1, ord(char)]]

        # ordenando
        ordenando = True
        i = 0

        while ordenando:
            if i == len(valores_ascii)-1:
                ordenando = False

            # ordenando por frequencia
            elif valores_ascii[i][0] > valores_ascii[i+1][0]:
                valores_ascii[i], valores_ascii[i+1] = \
                    valores_ascii[i+1], valores_ascii[i]
                i = 0
                continue

            # ordenando por valor ascii
            elif valores_ascii[i][0] == valores_ascii[i+1][0]:
                if valores_ascii[i][1] < valores_ascii[i+1][1]:
                    valores_ascii[i], valores_ascii[i+1] = \
                        valores_ascii[i+1], valores_ascii[i]
                    i = 0
                    continue
            i += 1

        # exibindo o resultado
        for item in valores_ascii:
            print(f"{item[1]} {item[0]}")
        print("")

    except EOFError:
        break
